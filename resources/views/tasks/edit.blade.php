                ...
                <form action="{{ route('tasks.update', $task->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="status" id="status" class="form-control @error('status') is-invalid @enderror">
                            <option value="pending" @if( old('satus', $task->status) == 'panding') selected @endif>Pending</option>
                            <option value="on progress" @if( old('satus', $task->status) == 'on progress') selected @endif>On Progress</option>
                        </select>
                        @error('due_date')
                            <div class="invalid-feadback">{{ $message }}</div>
                        @enderror
